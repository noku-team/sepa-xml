...
function SepaXML(format, isSwiss) {
...
SepaXML.prototype = {
  ...
  addTransaction: function(payment) {
    if(!this.isSwiss) {
      ...
      if (!payment || !payment.amount || !payment.id) return false;
    }

    if(!this.isSwiss) {
      if(!payment.iban || !payment.name) {
        return false
      }
    ...
    if (!payment.ccy) payment.ccy = "EUR";
    if (!payment.method) payment.method = "TRF";
    ...
    this._payments.transactions.push({
      ...
      bic: payment.bic,
      typeInformation: payment.typeInformation,
      ccy: payment.ccy,
      amount: payment.amount,
      creditorName: payment.creditorName,
      creditorAddress: payment.creditorAddress,
